...
namespace ClipboardCanvas.Sdk.Extensions
{
    public static class NavigationExtensions
    {
        ...
        public static async Task<bool> TryNavigateAsync<T>(this INavigationService navigationService, Func<Task<T>>? initializer = null, bool useInitialization = true)
            where T : class, IViewDesignation
        {
            var view = navigationService.TryGetView<T>();
            var isNewView = view is null;

            if (view is null && initializer is not null)
                view = await initializer.Invoke();
            else
                return false;
